...
static	int	FIP_n_states = sizeof(FIP_states)/sizeof(FIP_states[0]);

const	FIP_STATE_T	*
FIPS_find_state(const char *fips)
{
	int	i, j, k, cv;

	for(i = 0, j = n_states - 1; i <= j; ){
		k = (i + j) / 2;
		if((cv = strcmp(FIP_states[k].s_fips, fips)) == 0)
			return &FIP_states[k];
		else if(cv < 0)
			i = k + 1;
		else
